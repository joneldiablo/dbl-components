# Estado de traducciones

## Revisados y traducidos

### Raíz

- `src/js/app-controller.js` → `ts/app-controller.ts`
- `src/js/complex-component.js` → `ts/complex-component.tsx`
- `src/js/complex-responsive-component.js` → `ts/complex-responsive-component.tsx`
- `src/js/component.js` → `ts/component.tsx`
- `src/js/components-server.js` → `ts/components-server.ts`
- `src/js/components.js` → `ts/components.ts`
- `src/js/containers.js` → `ts/containers.ts`
- `src/js/controller.js` → `ts/controller.ts`
- `src/js/controllers.js` → `ts/controllers.ts`
- `src/js/index.js` → `ts/index.ts`
- `src/js/json-render-component.js` → `ts/json-render-component.tsx`
- `src/js/json-render.js` → `ts/json-render.ts`
- `src/js/prop-types.js` → `ts/prop-types.ts`
- `src/js/views.js` → `ts/views.ts`

### actions

- `src/js/actions/action.js` → `ts/actions/action.tsx`

### containers

- `src/js/containers/container.js` → `ts/containers/container.tsx`
- `src/js/containers/flex-container.js` → `ts/containers/flex-container.tsx`
- `src/js/containers/floating-container/floating-container.jsx` → `ts/containers/floating-container.tsx`
- `src/js/containers/grid-container.js` → `ts/containers/grid-container.tsx`
- `src/js/containers/json-render-container.js` → `ts/containers/json-render-container.tsx`
- `src/js/containers/list-container.js` → `ts/containers/list-container.tsx`
- `src/js/containers/proportional-container.js` → `ts/containers/proportional-container.tsx`
- `src/js/containers/tabs-container.js` → `ts/containers/tabs-container.tsx`
- `src/js/containers/alert-container/alert-container.js` → `ts/containers/alert-container/alert-container.tsx`
- `src/js/containers/auto-responsive-container.js` → `ts/containers/auto-responsive-container.tsx`
- `src/js/containers/card-container.js` → `ts/containers/card-container.tsx`
- `src/js/containers/details-container.js` → `ts/containers/details-container.tsx`
- `src/js/containers/dnd-list-container.js` → `ts/containers/dnd-list-container.tsx`
- `src/js/containers/dropdown-button-container.js` → `ts/containers/dropdown-button-container.tsx`
- `src/js/containers/dropdown-container.js` → `ts/containers/dropdown-container.tsx`
- `src/js/containers/fetch-container.js` → `ts/containers/fetch-container.tsx`
- `src/js/containers/footer-container.js` → `ts/containers/footer-container.tsx`
- `src/js/containers/form-container.js` → `ts/containers/form-container.tsx`
- `src/js/containers/fullscreen-container.js` → `ts/containers/fullscreen-container.tsx`
- `src/js/containers/grid-switch-container.js` → `ts/containers/grid-switch-container.tsx`
- `src/js/containers/hero-container.js` → `ts/containers/hero-container.tsx`
- `src/js/containers/modal-button-container.js` → `ts/containers/modal-button-container.tsx`
- `src/js/containers/modal-container.js` → `ts/containers/modal-container.tsx`
- `src/js/containers/offcanvas/offcanvas.js` → `ts/containers/offcanvas/offcanvas.tsx`
- `src/js/containers/panel-container/panel-container.js` → `ts/containers/panel-container/panel-container.tsx`
- `src/js/containers/scroll-container.jsx` → `ts/containers/scroll-container.tsx`
- `src/js/containers/slide-container.js` → `ts/containers/slide-container.tsx`
- `src/js/containers/table-container.js` → `ts/containers/table-container.tsx`

### forms

- `src/js/forms/fields.js` → `ts/forms/fields/index.ts`
- `src/js/forms/fields/field.js` → `ts/forms/fields/field.tsx`
- `src/js/forms/fields/select-field.js` → `ts/forms/fields/select-field.tsx`
- `src/js/forms/form.js` → `ts/forms/form.tsx`
- `src/js/forms/groups.js` → `ts/forms/groups/index.ts`
- `src/js/forms/groups/card-group.js` → `ts/forms/groups/card-group.tsx`
- `src/js/forms/groups/grid-group.js` → `ts/forms/groups/grid-group.tsx`
- `src/js/forms/groups/group.js` → `ts/forms/groups/group.tsx`

### hooks

- `src/js/hooks/use-classes.js` → `ts/hooks/use-classes.ts`
- `src/js/hooks/use-event-handler.js` → `ts/hooks/use-event-handler.ts`

### i18n

- `src/js/i18n/index.js` → `ts/i18n/index.tsx`
- `src/js/i18n/select-language.js` → `ts/i18n/select-language.tsx`

### konva

- `src/js/konva/index.js` → `ts/konva/index.tsx`

### media

- `src/js/media/icons.js` → `ts/media/icons.tsx`
- `src/js/media/image.js` → `ts/media/image.tsx`
- `src/js/media/svg-imports.jsx` → `ts/media/svg-imports.tsx`
- `src/js/media/svg.js` → `ts/media/svg.tsx`
- `src/js/media/video.js` → `ts/media/video.tsx`
- `src/js/media/youtube-video.js` → `ts/media/youtube-video.tsx`

### chartjs

- `src/js/chartjs/_chartjs.jsx` → `ts/chartjs/_chartjs.tsx`
- `src/js/chartjs/bar-chartjs.jsx` → `ts/chartjs/bar-chartjs.tsx`
- `src/js/chartjs/doughnut-chartjs.jsx` → `ts/chartjs/doughnut-chartjs.tsx`
- `src/js/chartjs/line-chartjs.jsx` → `ts/chartjs/line-chartjs.tsx`
- `src/js/chartjs/scatter-chartjs.jsx` → `ts/chartjs/scatter-chartjs.tsx`

### navigation

- `src/js/navigation/brand-navigation.js` → `ts/navigation/brand-navigation.tsx`
- `src/js/navigation/card-list-navigation.js` → `ts/navigation/card-list-navigation.tsx`
- `src/js/navigation/cards-navigation.js` → `ts/navigation/cards-navigation.tsx`
- `src/js/navigation/header-navigation.js` → `ts/navigation/header-navigation.tsx`
- `src/js/navigation/menu-item/menu-item.js` → `ts/navigation/menu-item/menu-item.tsx`
- `src/js/navigation/navbar.js` → `ts/navigation/navbar.tsx`
- `src/js/navigation/navigation.js` → `ts/navigation/navigation.tsx`
- `src/js/navigation/react-router-link.js` → `ts/navigation/react-router-link.tsx`
- `src/js/navigation/react-router-navlink.js` → `ts/navigation/react-router-navlink.tsx`
- `src/js/navigation/service-list-navigation.js` → `ts/navigation/service-list-navigation.tsx`
- `src/js/navigation/side-navigation.js` → `ts/navigation/side-navigation.tsx`

### react-router-schema

- `src/js/react-router-schema/react-router-schema.jsx` → `ts/react-router-schema/react-router-schema.tsx`
- `src/js/react-router-schema/route.jsx` → `ts/react-router-schema/route.tsx`
- `src/js/react-router-schema/with-route-wrapper.jsx` → `ts/react-router-schema/with-route-wrapper.tsx`

### views

- `src/js/views/title-view.js` → `ts/views/title-view.tsx`
- `src/js/views/view.js` → `ts/views/view.tsx`

## Pendientes de traducción

### bootstrap

- *Omitido según acuerdo: no se migrará esta carpeta.*
- `src/js/bootstrap/components/accordion/accordion.js`
- `src/js/bootstrap/components/alert/alert.js`
- `src/js/bootstrap/components/badge/badge.js`
- `src/js/bootstrap/components/breadcrumb/breadcrumb.js`
- `src/js/bootstrap/components/button-group/button-group.js`
- `src/js/bootstrap/components/button/button.js`
- `src/js/bootstrap/components/card/card.js`
- `src/js/bootstrap/components/carousel/carousel.js`
- `src/js/bootstrap/components/close-button/close-button.js`
- `src/js/bootstrap/components/collapse/collapse.js`
- `src/js/bootstrap/components/dropdowns/dropdowns.js`
- `src/js/bootstrap/components/list-group/list-group.js`
- `src/js/bootstrap/components/modal/modal.js`
- `src/js/bootstrap/components/nav-tab/nav-tab.js`
- `src/js/bootstrap/components/navbar/navbar.js`
- `src/js/bootstrap/components/offcanvas/offcanvas.js`
- `src/js/bootstrap/components/pagination/pagination.js`
- `src/js/bootstrap/components/placeholder/placeholder.js`
- `src/js/bootstrap/components/popover/popover.js`
- `src/js/bootstrap/components/progress/progress.js`
- `src/js/bootstrap/components/scrollspy/scrollspy.js`
- `src/js/bootstrap/components/spinner/spinner.js`
- `src/js/bootstrap/components/toast/toast.js`
- `src/js/bootstrap/components/tooltip/tooltip.js`
- `src/js/bootstrap/content/figure/figure.js`
- `src/js/bootstrap/content/image/image.js`
- `src/js/bootstrap/content/table/table.js`
- `src/js/bootstrap/forms/check/check.js`
- `src/js/bootstrap/forms/floating-label/floating-label.js`
- `src/js/bootstrap/forms/form-control/form-control.js`
- `src/js/bootstrap/forms/input-group/input-group.js`
- `src/js/bootstrap/forms/radio/radio.js`
- `src/js/bootstrap/forms/range/range.js`
- `src/js/bootstrap/forms/select/select.js`
- `src/js/bootstrap/index.js`

### containers

- *Completado*

### draw

- `src/js/draw/trapezoid.js`

### forms

- `src/js/forms/fields/autocomplete-field.js`
- `src/js/forms/fields/checkbox-field.js`
- `src/js/forms/fields/date-range-field.js`
- `src/js/forms/fields/drop-file-field.js`
- `src/js/forms/fields/file-button-field.js`
- `src/js/forms/fields/file-field.js`
- `src/js/forms/fields/group-field.js`
- `src/js/forms/fields/hidden-field.js`
- `src/js/forms/fields/json-editor-field.js`
- `src/js/forms/fields/material/field.js`
- `src/js/forms/fields/material/info.js`
- `src/js/forms/fields/material/select-badges-field.js`
- `src/js/forms/fields/material/select-field.js`
- `src/js/forms/fields/new-password-field.jsx`
- `src/js/forms/fields/no-wrap-field.js`
- `src/js/forms/fields/pagination-field.js`
- `src/js/forms/fields/radio-field.js`
- `src/js/forms/fields/range-field.js`
- `src/js/forms/fields/switch-field.js`
- `src/js/forms/fields/textarea-field.js`

### sliders

- `src/js/sliders/hero.js`

### tables

- `src/js/tables/table.jsx`

### tools

- `src/js/tools/purgecss.js`

## Observaciones de revisión

- `ts/components.ts`, `ts/containers.ts`, `ts/controllers.ts`, `ts/forms/fields/index.ts`: aún dependen de módulos en `src/js`; habrá que migrar esas dependencias antes de poder eliminar el árbol JavaScript.
- Carpeta `src/js/bootstrap`: se omite su migración porque los componentes generados no se utilizan (automatización fallida).
